...
Step 2: ui-ux-pro-max supplements with chart type recommendations:
...
> **路径说明：** 本文档中所有 `scripts/search.py` 路径均相对于本 skill 根目录（即本 SKILL.md 所在目录）。执行前请按实际安装位置补全前缀，例如 `.agents/skills/ui-ux-pro-max/scripts/search.py` 或 `skills/ui-ux-pro-max/scripts/search.py`。
